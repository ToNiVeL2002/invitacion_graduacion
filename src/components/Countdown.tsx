  ...
  return (
    <div className="flex flex-col items-center justify-center gap-2.5 mx-auto">
      ...
      <div
        ...
        style={{
          background: 'linear-gradient(145deg, #0B2341 0%, #143A63 100%)',
          border: '1.5px solid rgba(200,162,74,0.45)',
          boxShadow:
            '0 10px 30px rgba(11,35,65,0.2), inset 0 1px 0 rgba(255,255,255,0.1)',
        }}
      >
      ...
      {/* Etiqueta */}
      <p className="font-montserrat text-xs sm:text-sm font-bold tracking-[0.25em] uppercase text-[#143A63] text-center">
        {label}
      </p>
    </div>
  )
...
export default function Countdown() {
  ...
  const targetDate = dayjs(eventData.eventDate)
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 className="font-cormorant text-4xl sm:text-5xl md:text-6xl font-light mb-4 text-[#0B2341] text-center w-full">
        ...
        {isOver && (
          <p className="font-lora text-2xl italic text-[#C8A24A] text-center w-full">
            {eventData.eventDateDisplay}
          </p>
        )}

        {/* Fecha */}
        <div className="gold-divider mx-auto mt-14 mb-4 max-w-[200px] sm:max-w-xs" />
        <p className="font-cinzel text-sm sm:text-base tracking-[0.2em] uppercase font-bold text-[#143A63] text-center w-full">
          {eventData.eventDateDisplay}
        </p>
      </motion.div>
    </section>
