...
export default function Hero() {
  const { graduate, heroQuote } = eventData
  ...
  return (
    <section
      id="hero"
      className="relative w-full min-h-screen flex flex-col items-center justify-center py-12 px-6 sm:px-12 md:px-16 overflow-hidden select-none bg-navy"
    >
      {/* ── Fondo Responsivo: Monumento en celular, Frontis en pantallas medianas/grandes ── */}
      {/* Fondo Móvil (Celular) */}
      <div
        className="block md:hidden absolute inset-0 z-0 bg-cover bg-center transition-transform duration-1000 scale-105"
        style={{ backgroundImage: `url(${graduate.monumento})` }}
      >
        <div
          className="absolute inset-0"
          style={{
            background:
              'radial-gradient(circle at center, rgba(11,35,65,0.35) 0%, rgba(11,35,65,0.65) 85%), linear-gradient(to bottom, rgba(11,35,65,0.25) 0%, rgba(11,35,65,0.75) 100%)',
          }}
        />
      </div>
      ...
      <div
        className="hidden md:block absolute inset-0 z-0 bg-cover bg-center transition-transform duration-1000 scale-105"
      ...
      <div className="relative z-20 flex flex-col items-center gap-2 text-center px-4 max-w-4xl w-full my-auto">
        ...
        >
          ...
          <p
            className="font-cinzel text-sm sm:text-base font-bold tracking-[0.3em] uppercase drop-shadow-md mt-1"
